"""
ECM 등가회로 모델 연구포털
rain0807.streamlit.app 완전 동일 디자인
오류 수정: session_state 키와 widget 키 완전 분리
"""
...
# ── SESSION STATE 초기화 ─────────────────────────────────────
# ⚠️ 중요: session_state 키는 위젯 key=와 절대 겹치지 않게 접두사 _ss_ 사용
def _init():
    defaults = {
        "_ss_page":       "home",
        "_ss_dark":       False,
...
def make_hero():
    ...
    for y2 in range(0, h, 80):
        draw.line([(0, y2), (w, y2)], fill=(255, 255, 255, 10))
    # 회로 라인 (틸)
    lc = (45, 212, 191, 65)
    segs = [
        (920,90,1100,90),(1100,90,1100,220),(1100,220,1260,220),
        (860,320,1020,320),(1020,320,1020,460),(1020,460,1180,460),
        (1120,130,1250,130),(1250,130,1250,260),
    ]
    for x0,y0,x1,y1 in segs:
        draw.line([(x0,y0),(x1,y1)], fill=lc, width=2)
    # 회로 노드
    for cx,cy in [(1100,90),(1100,220),(1020,320),(1020,460)]:
        draw.ellipse([cx-7,cy-7,cx+7,cy+7], fill=(45,212,191,110))
    return _b64(img.filter(ImageFilter.GaussianBlur(1)))

@st.cache_data
def make_why_photo():
    """WHY 섹션 왼쪽 이미지 — 테슬라 내부 분위기"""
    w, h = 760, 500
    img = Image.new("RGB", (w, h))
    draw = ImageDraw.Draw(img)
    # 밝은 배경 (차량 내부 분위기)
    for y in range(h):
        ...
        draw.line([(0,y),(w,y)], fill=(r,g,b))
    # 대시보드
    draw.rounded_rectangle([60,210,700,430], radius=18,
                            fill=(28,38,58), outline=(55,75,115), width=2)
    # 화면
    draw.rounded_rectangle([290,238,680,415], radius=12,
                            fill=(18,28,46), outline=(45,212,191,80), width=1)
    # 화면 내 그래프 라인
    for i, y_v in enumerate(range(265,400,40)):
        alpha = 200 - i*30
        draw.line([(300,y_v),(670,y_v)], fill=(45,212,191,max(30,alpha)), width=1)
    # 그래프 데이터 라인
    pts = [(300,380),(340,340),(380,355),(420,310),(460,330),(500,290),(540,305),(580,265),(620,280),(660,260)]
    draw.line(pts, fill=(45,212,191,200), width=3)
    # 핸들
    draw.ellipse([100,280,255,390], outline=(75,95,125), width=3)
    draw.line([(177,280),(177,390)], fill=(75,95,125), width=2)
    draw.line([(100,335),(255,335)], fill=(75,95,125), width=2)
...
def make_topic_bg(idx):
    """토픽 카드 배경 — 어두운 그라디언트"""
    palettes = [
        ((12,22,44),(18,40,80)),    # t01 딥블루
        ((8,38,58),(12,60,95)),     # t02 오션
        ((38,12,12),(72,22,22)),    # t03 레드
        ((12,38,28),(18,68,48)),    # t04 그린
        ((38,28,8),(68,52,12)),     # t05 앰버
        ((18,12,48),(32,22,88)),    # t06 퍼플
        ((8,32,42),(12,58,78)),     # t07 틸
        ((32,12,38),(58,22,68)),    # t08 마젠타
        ((8,38,18),(12,68,32)),     # t09 포레스트
        ((42,18,8),(78,32,12)),     # t10 오렌지
        ((12,12,48),(22,22,88)),    # t11 인디고
        ((38,8,28),(68,15,52)),     # t12 와인
        ((8,32,32),(12,62,62)),     # t13 다크틸
        ((32,32,8),(62,62,12)),     # t14 올리브
    ]
    ...
    # 회로 장식
    draw.rectangle([55,75,175,155], outline=(255,255,255,55), width=2)
    draw.line([(175,115),(240,115)], fill=(255,255,255,55), width=2)
    draw.line([(55,115),(18,115)],   fill=(255,255,255,45), width=2)
    draw.ellipse([238,108,256,122],  outline=(255,255,255,55), width=2)
    return _b64(img)
...
TOPICS = [
    {"id":"t01","num":"01","title":"개방회로 전압 (OCV)",
     ...
     "kps":[
         {"i":"⚡","t":"정의","d":"전류 i=0 상태의 단자 전압. 순수 전기화학적 평형 전압."},
     ...
     "eqs":[
         {"t":"이상 전압원 모델","c":"v(t) = OCV(z(t))  // i=0 가정"},
         {"t":"온도 보정 모델",  "c":"OCV(z,T) = OCV₀(z) + T·OCVrel(z)"},
     ],
     "comp":{"h":["항목","NMC","LFP"],
             "r":[["OCV 형태","단조 S곡선","극평탄(3.33V)"],
                  ["히스테리시스","<10mV","50~100mV"],
                  ["SoC 추정 난이도","보통","어려움"]]},
    ...
    {"id":"t04","num":"04","title":"확산 전압 (RC 회로)",
     ...
     "kps":[
         {"i":"🌊","t":"물리적 원인","d":"전극 내 리튬 이온 농도 구배. 분 단위의 느린 시간 상수."},
    ...
    {"id":"t07","num":"07","title":"히스테리시스 전압",
     ...
     "eqs":[
         {"t":"동적 히스테리시스","c":"h[k+1] = exp(−|η·i·γ·Δt/Q|)·h[k]\n        − (1−exp(···))·sgn(i[k])\nγ ≈ 90"},
         {"t":"히스테리시스 전압","c":"V_hys[k] = M₀·s[k] + M·h[k]"},
     ],
     "comp":{"h":["특성","히스테리시스","RC 확산"],
             "r":[["시간 의존","없음","있음(소멸)"],
                  ["LFP 크기","50~100mV","수십mV"],
                  ["NMC 크기","<10mV","수십mV"]]},
     "refs":[("Dreyer 2010","10.1038/nmat2718")]},

    {"id":"t08","num":"08","title":"향상된 자가 교정 셀 모델 (ESC)",
     "sub":"RMSE=5.37mV 검증 완료",
     "tags":["ESC","상태공간","EKF"],
     "desc":"OCV+R₀+RC+히스테리시스를 통합한 최종 완성 모델.",
     "kps":[
         {"i":"🎯","t":"통합 구조","d":"상태벡터 x=[z,i_R,h]ᵀ. SoC·RC전류·히스테리시스 통합."},
     ...
     "eqs":[
         {"t":"ESC 상태방정식","c":"x[k+1] = A[k]·x[k] + B[k]·u[k]\n// x=[z,i_R,h]ᵀ"},
    ...
    {"id":"t09","num":"09","title":"셀 데이터 수집 실험실 장비",
     "sub":"Arbin BT2043 + 환경 챔버",
     "tags":["Arbin","환경챔버","장비"],
     "desc":"12채널 독립 테스트 시스템과 −45°C~190°C 환경 챔버.",
     "kps":[
         {"i":"🔬","t":"Arbin BT2043","d":"12채널 독립. 켈빈 4선식. ±20A/0~5V."},
     ...
     "comp":{"h":["장비","사양","용도"],
             "r":[["Arbin BT2043","12ch독립,±20A","충방전"],
                  ["환경 챔버","−45~190°C","온도 제어"]]},
     "refs":[("Plett 2015 pp.65-67","")]},

    {"id":"t10","num":"10","title":"OCV 관계 실험실 테스트",
     "sub":"4-스크립트 OCV 취득법",
     "tags":["4-스크립트","C/30","GITT"],
     "desc":"C/30 저속 방전으로 OCV(z,T) 룩업 테이블 완성.",
     "kps":[
         {"i":"1️⃣","t":"Script 1","d":"테스트 온도 T에서 C/30으로 v_min까지 방전."},
     ...
     "comp":{"h":["절차","온도","목적"],
             "r":[["Script 1","테스트 T","방전 OCV"],
                  ["Script 2","25°C","기준점 확인"],
                  ["Script 3","테스트 T","충전 OCV"],
                  ["Script 4","25°C","기준점 확인"]]},
     "refs":[("Weppner 1977","10.1149/1.2133152")]},

    {"id":"t11","num":"11","title":"동적 관계 실험실 테스트",
     "sub":"UDDS 프로파일 파라미터 추출",
     "tags":["UDDS","시스템식별","γ"],
     "desc":"UDDS 주행 사이클로 RC·히스테리시스 파라미터 추출.",
     "kps":[
         {"i":"🚗","t":"UDDS 프로파일","d":"도심 주행 패턴 모사 전류 프로파일."},
     ...
     "comp":{"h":["절차","내용"],
             "r":[["Script 1","완충 후 OCV 측정"],
                  ["Script 2","UDDS 방전"],
                  ["Script 3","UDDS 충전"]]},
     "refs":[("Seaman 2014","10.1016/j.jpowsour.2013.11.037")]},

    {"id":"t12","num":"12","title":"모델링 결과 예시",
     "sub":"25Ah NMC UDDS 10시간 검증",
     "tags":["RMSE","UDDS","NMC"],
     "desc":"RMSE=5.37mV 달성. 온도별 파라미터 특성 분석.",
     "kps":[
         {"i":"✅","t":"RMSE 5.37mV","d":"25Ah NMC, UDDS 10h. 교재 목표 충족."},
    ...
    {"id":"t13","num":"13","title":"결론 및 향후 방향",
     "sub":"DFN+ML 하이브리드 차세대",
     "tags":["한계","DFN","ML"],
     "desc":"ESC 한계와 물리 기반·ML 하이브리드 차세대 방향.",
     "kps":[
         {"i":"⚠️","t":"ESC 한계","d":"노화 예측 불가. 덴드라이트 임계점 예측 불가."},
    ...
    {"id":"t14","num":"14","title":"MATLAB ESC 모델 툴박스",
     "sub":"simCell · processOCV · processDynamic",
     "tags":["simCell","processOCV","MATLAB"],
     "desc":"ESCtoolbox 핵심 함수와 사용법. mocha-java.uccs.edu/BMS1/",
     "kps":[
         {"i":"💻","t":"simCell.m","d":"ESC 시뮬레이션. 전류 프로파일 → 모델 전압 출력."},
     ...
     "comp":{"h":["함수","입력","출력"],
             "r":[["processOCV","OCV 데이터","OCV₀,OCVrel"],
                  ["processDynamic","동적 데이터","R₀,R₁,C₁,γ,M,M₀"],
                  ["simCell","전류+model","전압 시뮬레이션"]]},
     "refs":[("Plett 2015 pp.82-87","")]},
]

# ── 뉴스 (배터리 등가회로 관련) ─────────────────────────────
NEWS = [
    {"title":"[수요광장]배터리 관리 시스템, 보안이 없으면 기능도 없다",
     "source":"KR 전기신문","date":"Tue, 31 Ma",
     "url":"https://www.electimes.com"},
    {"title":"[포커스] LG엔솔, 소프트웨어·서비스 중심 기업으로의 전략적 전환...왜?",
     "source":"KR 투데이에너지","date":"Sun, 05 Ap",
     "url":"https://www.todayenergy.kr"},
    {"title":"제주대학교 2025년 초기창업패키지 선정기업 — 전기차 배터리 AI 플랫폼 개발 '퀀텀하이텍'",
     "source":"KR 한경매거진","date":"Wed, 22 Oc",
     "url":"https://magazine.hankyung.com"},
    {"title":"[2025년 딥테크 강소기업 탐방]〈4〉 휴컨 - 전자신문",
     "source":"KR 전자신문","date":"Wed, 15 Ja",
...
PAPERS = [
    {"idx":"01","tag":"교재",
     ...
     "journal":"Artech House, 2015. ISBN: 978-1-60807-650-3",
     "doi":"","url":"https://www.artechhouse.com",
     "desc":"ESC 등가회로 모델의 표준 교재. OCV·SoC·RC·히스테리시스 통합 모델링의 원전."},
    {"idx":"02","tag":"핵심 논문",
     "title":"Extended Kalman filtering for battery management systems of LiPB-based HEV battery packs: Part 1",
     "authors":"G. L. Plett",
     "journal":"Journal of Power Sources, 134(2), 252–261, 2004",
     "doi":"10.1016/j.jpowsour.2004.02.031",
     "url":"https://doi.org/10.1016/j.jpowsour.2004.02.031",
     "desc":"ESC 모델과 EKF를 결합한 SoC 자가 교정 추정법의 원조 논문."},
    {"idx":"03","tag":"리뷰 논문",
    ...
    {"idx":"04","tag":"EIS 분석",
     "title":"A survey of some equivalent circuits for describing the electrical behavior of lithium‐ion batteries",
    ...
    {"idx":"08","tag":"EKF+ML",
     ...
     "desc":"BPNN과 EKF를 결합한 SoC 추정. 등가회로 모델 파라미터 실시간 갱신."},
]
...
.rn-links{{display:flex;align-items:center;gap:52px;}}
.rn-link{{font-size:13px;font-weight:500;color:{GRAY};letter-spacing:.01em;}}
...
.rn-hero-ov{{
    ...
    background:linear-gradient(135deg,
        rgba(7,14,24,.94) 0%,rgba(10,20,42,.88) 40%,
        rgba(12,26,55,.80) 70%,rgba(13,30,62,.72) 100%);
}}
.rn-hero-grid{{
    position:absolute;inset:0;
    background-image:
...
.rn-hero-body{{
    ...
    padding:120px 80px 80px;max-width:720px;
}}
.rn-eyebrow{{
    font-size:11px;letter-spacing:.22em;color:{TEAL};
...
.rn-h1{{
    ...
    line-height:1.02;color:#fff;letter-spacing:-.035em;margin-bottom:0;
}}
.rn-h1-teal{{color:{TEAL};display:block;}}
.rn-sub{{
    font-size:15px;color:rgba(255,255,255,.52);
    margin-top:26px;line-height:1.82;max-width:500px;font-weight:300;
}}
.rn-scroll{{
    position:absolute;bottom:44px;right:56px;
    color:rgba(255,255,255,.22);font-size:10px;
    letter-spacing:.24em;text-transform:uppercase;
    writing-mode:vertical-rl;
    display:flex;align-items:center;gap:10px;
}}
.rn-scroll::after{{
    content:'';display:inline-block;width:1px;height:66px;
    background:linear-gradient(to bottom,rgba(255,255,255,.18),transparent);
}}

/* ── WHY SECTION (rain0807 이미지+텍스트 레이아웃) ── */
...
.rn-why-title{{
    font-size:46px;font-weight:900;color:{NAVY};
    line-height:1.08;margin-bottom:38px;letter-spacing:-.035em;
}}
.rn-why-item{{display:flex;gap:24px;align-items:flex-start;margin-bottom:32px;}}
.rn-why-num{{
    font-size:28px;font-weight:900;color:{TEAL};opacity:.45;
    min-width:44px;line-height:1.1;font-family:'Roboto Mono',monospace;
}}
...
.rn-tech-title{{
    font-size:52px;font-weight:900;color:{NAVY};
    line-height:1.04;letter-spacing:-.035em;
}}
.rn-tech-desc{{font-size:14px;color:{GRAY};line-height:1.8;padding-top:68px;}}

/* ── TOPIC CARD ── */
.rn-tc{{position:relative;overflow:hidden;cursor:pointer;transition:all .3s;}}
.rn-tc:hover{{transform:scale(1.01);z-index:2;}}
.rn-tc img{{width:100%;object-fit:cover;display:block;transition:transform .4s;}}
.rn-tc:hover img{{transform:scale(1.04);}}
.rn-tc-ov{{
    position:absolute;inset:0;
...
.rn-tc-chip{{
    font-size:9px;letter-spacing:.22em;color:{TEAL};
...
.rn-news-title{{
    font-size:58px;font-weight:900;color:{NAVY};
...
.rn-paper-card{{
    background:#fff;border:1px solid {BD};border-radius:10px;
...
.sb-head{{
    ...
    padding:13px 15px;border-radius:10px;margin-bottom:13px;
}}
...
.sb-val{{
    font-family:'Roboto Mono',monospace;font-size:12px;
    color:{TEAL};line-height:2;
}}
.sb-pb{{height:6px;background:{BD};border-radius:3px;overflow:hidden;}}
.sb-pb-fill{{
    height:100%;background:linear-gradient(90deg,{TEAL},#0891b2);
...
.kp-card{{
    ...
    border-radius:0 10px 10px 0;padding:13px;margin-bottom:10px;
}}
...
[data-testid="metric-container"]{{
    background:#fff!important;border:1px solid {BD}!important;
    border-radius:10px!important;border-left:3px solid {TEAL}!important;padding:13px!important;
}}
...
with st.sidebar:
    ...
    with st.expander("⚡ RC 파라미터 계산", expanded=True):
        # 위젯 key: w_로 시작 → session_state와 충돌 없음
    ...
    st.markdown("---")
    st.markdown(f'<div style="font-size:12px;font-weight:700;color:{NAVY};margin-bottom:6px;">📝 메모장</div>',
                unsafe_allow_html=True)
    # 위젯 key: w_memo_ta → _ss_memo와 다름
    memo_val = st.text_area("메모", value=gs("_ss_memo"), height=100,
                             key="w_memo_ta", label_visibility="collapsed")
    mbc1, mbc2 = st.columns(2)
    with mbc1:
        if st.button("💾 저장", key="w_memo_save", use_container_width=True):
            ss("_ss_memo", memo_val)
            st.toast("저장됨!")
    with mbc2:
    ...
    st.markdown("---")
    done = sum(1 for v in gs("_ss_progress").values() if v)
    st.markdown(f"""
    <div style="font-size:12px;font-weight:700;color:{NAVY};margin-bottom:6px;">
        📈 학습 진도 <span style="color:{TEAL};">{done}/{len(TOPICS)}</span>
    </div>
    <div class="sb-pb"><div class="sb-pb-fill" style="width:{done/len(TOPICS)*100:.0f}%;"></div></div>
    <div style="font-size:10px;color:{LGRAY};margin-top:4px;">섹션 카드를 열면 자동 저장됩니다</div>
    """, unsafe_allow_html=True)
    dc = st.columns(7)
    for di2, tp in enumerate(TOPICS):
        with dc[di2 % 7]:
            c = TEAL if gs("_ss_progress").get(tp["id"], False) else BD
            st.markdown(f'<div style="height:4px;border-radius:2px;background:{c};margin-top:6px;" title="{tp["title"]}"></div>',
                        unsafe_allow_html=True)
    st.markdown("---")
    st.markdown(f'<div style="font-size:11px;font-weight:600;color:{NAVY};margin-bottom:5px;">🔗 관련 자료</div>',
                unsafe_allow_html=True)
...
<div class="rn-nav">
    ...
    <div class="rn-links">
        <span class="rn-link">연구 개요</span>
        <span class="rn-link">핵심 기술</span>
        <span class="rn-link">뉴스룸</span>
        <span class="rn-link">14개 주제</span>
    </div>
    <div class="rn-right">
...
# ── 네비게이션 버튼 ─────────────────────────────────────────
# ⚠️ 모든 버튼 key는 w_nav_ 접두사 → session_state와 충돌 없음
...
if page == "home":
        ...
        <div class="rn-why-grid">
            <div>
                <div class="rn-why-img">
                    <img src="data:image/png;base64,{WHY}"
    ...
    # TECH SECTION (rain0807 핵심 기술 — 2열 풀폭 카드)
    st.markdown(f"""
    <div class="rn-tech">
    ...
    # 2열 풀폭 카드 (TOPIC 01 & 07)
    col_l, col_r = st.columns(2, gap="small")
    for col_idx, tp_idx in enumerate([0, 6]):  # t01, t07
        tp = TOPICS[tp_idx]
        ...
        with col:
            ...
            <div class="rn-tc" style="height:400px;margin-top:-46px;margin-bottom:4px;pointer-events:none;">
                <img src="data:image/png;base64,{card_bg}" style="height:400px;">
    ...
    for ni, news in enumerate(NEWS):
        ...
        for y in range(nh):
            t = y/nh
            ndraw.line([(0,y),(nw,y)],
                       fill=tuple(int(c1n[k]+t*(c2n[k]-c1n[k])) for k in range(3)))
        nb64 = _b64(nimg)
        st.markdown(f"""
            <div class="rn-nc">
                <div class="rn-nc-img">
    ...
    for p in PAPERS:
        ...
        st.markdown(f"""
        <div class="rn-paper-card" style="margin:0 80px 12px;">
...
# ── TOPICS ────────────────────────────────────────────────────
    ...
    st.markdown(f'<div style="font-size:32px;font-weight:900;color:{NAVY};letter-spacing:-.03em;margin-bottom:18px;">📖 주제별 학습 — 14개 섹션</div>',
                unsafe_allow_html=True)
    tnames = [f"Section {tp['num']}: {tp['title']}" for tp in TOPICS]
    cur_id = gs("_ss_topic")
    si = next((ti for ti, tp in enumerate(TOPICS) if tp["id"] == cur_id), 0)

    # 위젯 key: w_tsel → _ss_topic와 다름
    ...
    tags_h = " ".join([
        f'<span style="background:rgba(45,212,191,.1);color:{TEAL};border:1px solid rgba(45,212,191,.3);'
        f'border-radius:20px;padding:2px 9px;font-size:10px;font-weight:700;">{t}</span>'
        for t in tp["tags"]
    ])
    ...
    with th:
        ...
        <div style="position:relative;border-radius:12px;overflow:hidden;margin-bottom:14px;">
            <img src="data:image/png;base64,{card_bg}"
                 style="width:100%;height:140px;object-fit:cover;display:block;">
            <div style="position:absolute;inset:0;
                 background:linear-gradient(135deg,rgba(7,14,24,.92),rgba(12,26,55,.78));
                ...
                <div style="font-family:'Roboto Mono',monospace;font-size:9px;
                     color:rgba(45,212,191,.9);letter-spacing:.18em;margin-bottom:6px;">SECTION {tp['num']}</div>
    ...
    with tabs[0]:
        ...
        for ki, kp in enumerate(tp["kps"]):
            with kc[ki % 2]:
                st.markdown(f'<div class="kp-card"><div class="kp-i">{kp["i"]}</div>'
                            f'<div class="kp-t">{kp["t"]}</div>'
                            f'<div class="kp-d">{kp["d"]}</div></div>',
                            unsafe_allow_html=True)
        st.markdown("---")
    ...
    with tabs[1]:
        for eq in tp["eqs"]:
            st.markdown(f"**{eq['t']}**")
            st.markdown(f'<div class="eq-box">{eq["c"].replace(chr(10),"<br>")}</div>',
                        unsafe_allow_html=True)
        st.markdown("")
    ...
    with tabs[2]:
        ...
        if tp["id"] == "t01":
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=sx,y=[3.0,3.40,3.65,3.75,3.82,3.89,3.97,4.05,4.12,4.18,4.20],
                                     name="NMC",line=dict(color=TEAL,width=2.5)))
            fig.add_trace(go.Scatter(x=sx,y=[2.8,3.20,3.30,3.32,3.33,3.33,3.34,3.35,3.36,3.50,3.65],
                                     name="LFP",line=dict(color="#60a5fa",width=2.5,dash="dash")))
            ...
            fig.add_trace(go.Scatter(x=t_rc,y=vo,name="V(t)",fill="tonexty",
                                     fillcolor="rgba(45,212,191,.06)",line=dict(color=TEAL,width=2.5)))
            ...
            fig=go.Figure()
            fig.add_trace(go.Scatter(x=list(sh2),y=[2.8,3.3,3.34,3.35,3.36,3.37,3.38,3.40,3.45,3.5,3.65],
                                     name="충전 OCV",line=dict(color=TEAL,width=2.5)))
            fig.add_trace(go.Scatter(x=list(sh2),y=[2.8,3.2,3.27,3.28,3.29,3.30,3.31,3.33,3.38,3.44,3.60],
                                     name="방전 OCV",line=dict(color="#60a5fa",width=2.5,dash="dash")))
            ...
            fig=go.Figure()
            fig.add_trace(go.Scatter(x=list(range(11)),y=meas,name="실측",mode="lines+markers",
                                     line=dict(color=LGRAY,dash="dot",width=2),marker=dict(size=5)))
            fig.add_trace(go.Scatter(x=list(range(11)),y=modl,name="ESC 모델",line=dict(color=TEAL,width=2.5)))
            fig.add_annotation(x=5,y=3.69,text="RMSE=5.37mV ✓",
                               bgcolor="#dcfce7",bordercolor="#86efac",font=dict(color="#166534",size=11))
    ...
    with tabs[3]:
        comp = tp.get("comp")
        if comp:
            st.dataframe(pd.DataFrame(comp["r"], columns=comp["h"]),
    ...
    with tabs[4]:
        for lbl, doi in tp.get("refs", []):
            ...
            st.markdown(f"""
            <div style="background:{BG2};border-left:3px solid {TEAL};border-radius:0 8px 8px 0;
                 padding:12px 14px;margin-bottom:9px;">
                <div style="font-family:'Roboto Mono',monospace;font-size:9px;font-weight:700;
                     color:{TEAL};margin-bottom:4px;">{lbl}</div>
    ...
    with st.expander("🗂 전체 14개 주제 목록"):
        ...
        for ti2, t2 in enumerate(TOPICS):
            with ac[ti2 % 4]:
                ...
                if st.button(f"{'✅' if done2 else '⬜'} {t2['num']}. {t2['title']}",
                             key=f"w_all_{t2['id']}", use_container_width=True):
                    ss("_ss_topic", t2["id"]); st.rerun()

# ── EQUATIONS ─────────────────────────────────────────────────
elif page == "equations":
    st.markdown(f'<div style="font-size:32px;font-weight:900;color:{NAVY};margin-bottom:18px;">🔢 핵심 수식 모음</div>',
                unsafe_allow_html=True)
    EQ_LIST = [
    ...
    for ei, eq in enumerate(EQ_LIST):
        with ec[ei % 2]:
            st.markdown(f"""
            <div style="background:#fff;border:1px solid {BD};border-left:4px solid {eq['lc']};
                 border-radius:10px;padding:16px;margin-bottom:14px;">
                <div style="font-family:'Roboto Mono',monospace;font-size:9px;font-weight:700;
                     color:{eq['lc']};letter-spacing:.12em;margin-bottom:5px;">{eq['n']}</div>
    ...
    st.markdown("---")
    st.markdown(f'<div style="font-size:18px;font-weight:700;color:{NAVY};margin-bottom:12px;">📊 전압 응답 시뮬레이터</div>',
                unsafe_allow_html=True)
    ...
    with es2:
        ...
        fg2.add_trace(go.Scatter(x=ts2,y=vs2,name="V(t)",fill="tonexty",
                                 fillcolor="rgba(45,212,191,.06)",line=dict(color=TEAL,width=2.5)))
        fg2.update_layout(**plot_lyt(title=f"V(t) | I={sI}A τ={tau_e:.0f}s",
                                     xaxis_title="시간(s)",yaxis_title="전압(V)"))
        st.plotly_chart(fg2, use_container_width=True)

# ── QUIZ ──────────────────────────────────────────────────────
elif page == "quiz":
    st.markdown(f'<div style="font-size:32px;font-weight:900;color:{NAVY};margin-bottom:18px;">📝 이해도 퀴즈</div>',
                unsafe_allow_html=True)
    if st.button("🔄 다시 시작", key="w_quiz_reset"):
        ss("_ss_quiz_idx",0); ss("_ss_quiz_score",0)
        ss("_ss_quiz_ans",False); ss("_ss_quiz_done",False); st.rerun()
    ...
    if gs("_ss_quiz_done"):
        ...
        st.markdown(f"""
        <div style="background:rgba(45,212,191,.07);border:1px solid rgba(45,212,191,.3);
        ...
        if not gs("_ss_quiz_ans"):
            ...
            for oi, opt in enumerate(q["opts"]):
                ...
                if oi == q["ans"]:   st.success(f"✅ {lb}")
                    ...
                    st.markdown(f'<div style="padding:9px 12px;border:1px solid {BD};'
                                f'border-radius:7px;color:{GRAY};margin:3px 0;">{lb}</div>',
                                unsafe_allow_html=True)
            st.markdown(f"""
            <div style="background:rgba(45,212,191,.07);border-left:3px solid {TEAL};
                 border-radius:0 8px 8px 0;padding:12px;margin:10px 0;">
                <b>💡 해설:</b> {q['exp']}
                <div class="eq-box" style="margin-top:8px;">{q['f']}</div>
            </div>
            """, unsafe_allow_html=True)
            if st.button("다음 문제 →", type="primary", use_container_width=True, key=f"w_qnext_{idx}"):
                ss("_ss_quiz_idx", idx+1); ss("_ss_quiz_ans", False)
                if gs("_ss_quiz_idx") >= len(QUIZ):
                    ss("_ss_quiz_done", True)
                st.rerun()

# ── BOOKMARKS ─────────────────────────────────────────────────
elif page == "bookmarks":
    bm_list = gs("_ss_bookmarks")
    st.markdown(f'<div style="font-size:32px;font-weight:900;color:{NAVY};margin-bottom:18px;">🔖 북마크 ({len(bm_list)}건)</div>',
                unsafe_allow_html=True)
    if not bm_list:
...
# ── NOTICES ───────────────────────────────────────────────────
    ...
    st.markdown(f'<div style="font-size:32px;font-weight:900;color:{NAVY};margin-bottom:18px;">📢 공지사항</div>',
                unsafe_allow_html=True)
    # 위젯 key: w_ntab_sel → _ss_notices와 다름
    nt = st.radio("", ["공지사항","업데이트","Q&A"], horizontal=True,
                  key="w_ntab_sel", label_visibility="collapsed")
    st.markdown("---")
    for n in gs("_ss_notices"):
        ...
        <div style="background:#fff;border:1px solid {BD};border-radius:10px;padding:14px 16px;margin-bottom:9px;">
            <span style="background:{bc3};color:{tc3};border:1px solid {tc3}33;font-size:10px;
                 font-weight:700;padding:2px 8px;border-radius:20px;font-family:'Roboto Mono',monospace;
                 letter-spacing:.06em;display:inline-block;margin-bottom:7px;">{ntype}</span>
